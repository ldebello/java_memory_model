...
Las acciones ""are bound together"" estan asociadas a las ejecucciones, que combinan las acciones con la definicion del orden que tienen con acciones relacionadas. Esto suena muy ""ivory-tower-esque"", mejor veamos unos ejemplos
...
Las acciones linkeadas entre si en PO no ""preclude being "reordered""". De hecho, es un poco confuso hablar de reordenamiento de acciones, por que uno probablemente intenta hablar de sentencias reordenadas en un programa, lo cual genera nuevas ejecucciones. Lo cual genera una nueva pregunta sobre si las ejecucciones generadas por este nuevo programa violan las "provisions"" del JMM.
...
Lo que queremos decir es que dado el simple esquema ""schematics"" de acciones y ejecucciones, se pueden construir un infinito numero de ejecucciones. Estas ejecucciones estan desenganchadas de una realidad especifica, solo son ""primordial soup"", conteniendo todas las construcciones posibles. En algun lado en este grupo se encuentran las ejecucciones que pueden explicar un posible resultado para el programa dado, y el conjunto de todas las ""and the set of all such plausible executions"" posibles ejecucciones cubre el grupo de todos los posibles resultados del programa.

---

image::images/jmm/039.png[]

Aqui es donde Program Order (PO) ""jumps in"". Para filtrar las ejecucciones, ""we can take to reason about the particular program"", tenemos reglas de consistencia intra-thread, lo cual elimina todas las acciones no relacionadas. Por ejemplo, en el ejemplo anterior, mientras que la ilustraccion es posible no refleja el programa original, despues de leer el valor 2 desde x, en ese caso debemos escribir 1 a y no en z.

---

image::images/jmm/040.png[]

Aqui tenemos una ilustraccion del filtro aplicado. La consistencia Intra-thread es el primer filtro de ejecucciones, esto es lo que la mayoria de las personas hacen de forma implicita en sus cabezas cuando se lidia con el JMM. A este punto se puede notar que el JMM no es un modelo constructivo ""is a non-constructive model"": No construimos la solucion de forma inductiba ""inductively"", pero en su lugar tomamos el conjunto entero de ejecucciones ""the entire realm of executions"" y filtramos aquellas que son interesantes para nosotros.
...
Ahora empezamos a construir partes del modelo que realmente ordenan cosas. En un modelo de memoria relajado, nosotros no ordenamos todas las acciones, solo ""impose"" imponemos un orden en un conjunto limitado de primitivas. En el JMM, esas primitivas son encapsuladas en sus respectivas acciones "Synchronization Actions".

---

image::images/jmm/042.png[]

Synchronization Order (SO) es total orden dentro del conjunto de todas las Synchronization Actions, aunque esto no es lo mas interesante de este orden. El JMM provee dos limitantes ""constraints"" adicionales:
* Consistencia SO-PO
* Consistencia SO
...
Las slides a continuacion son auto-explicativas, y ""we’ll just skip them over"":
...
Necesitamos algo para conectar el estado de los threads, ""something which will drag the non-SA values along"". SO no es usado para esto, porque no es claro cuando y como ""drags the state along"". Por lo cual, necesitamos ""clear-cut suborder of SO"" que describa el data flow. Esto es llamado synchronizes-with order (SW).
...
SW solo juntas los pares de acciones especificas las cuales se "ven" unas a las otras. Mas formalmente el write sobre g "synchronizes-with" todos los subsiguientes reads en g. Subsiguiente es definido en terminos de SO, y por lo tanto en base a la consistencia SO, el write de "1" solo se sincroniza con la lectura de "1". En este ejemplo vemos el SW entre las acciones. Este suborden nos brinda el "bridge" entre threads, pero aplicado a SA. Extendamos esto a otras acciones.
...
Ahora, si construimos la union entre PO y SW, y luego cerramos esa union, obtenemos el orden derivado: Happens-Before (HB). HB en este sentido adquiere ambas semanticas Inter-Thread y Intra-Thread. PO ""leaks"" la informacion acerca de las sequential actions en cada thread en HB. y SW ""leaks"" cuando el estado "synchronizes". HB es orden parcial, y nos permite la construccion de ejecucciones equivalentes con reordenamientos de acciones.

---

image::images/jmm/059.png[]

Happen-before viene con otra regla de consistencia. Si recordamos la regla de consistencia de SO, que establece que la sincronizacion de acciones debe ver el ultimo write en SO. La consistencia de Happens-before es similar aplicada al orden del tipo HB. Esto ""dictates"" dicta que los writes pueden ser observados por cualquier read en particular.
...
La primer parte ""is rather relaxing"": Podemos observar los write que pasaron antes que nosotros, o cualquier otro unordered write. Esta es una propiedad muy importante del modelo: nosotros especificamente permitimos "races", porque "races" suceden en el mundo real. Si estos fueran prohibidos, los runtimes tendrian una tarea muy complicada optimizando el codigo porque ellos necesitarian forzar el orden en todos lados.
Notemos como esto nos elimina la posibilidad de ver ""writes ordered"" luego de leer en HB orden.

---

image::images/jmm/062.png[]

La segunda parte agregar algunas restricciones adicionales sobre la visibilidad de los ""preceding writes"": Solo podemos ver el ultimo write en HB orden. Cualquier otro write anterior es invisible para nosotros. Por lo tanto, en la ausencia de "races", solo podemos ver el ultimo write en HB.

---

image::images/jmm/063.png[]

La consecuencia de la consistencia de HB es para filtrar otro subset de ejecucciones que observan algo que esta permitido de ser observado. HB se extiende sobre las acciones "non-synchronized", y por lo tanto deja al modelo ""embrace"" todas las acciones en sus ejecucciones.
